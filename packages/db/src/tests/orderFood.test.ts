...
import { OrderPaymentMethod, OrderPaymentStatus, OrderStatus } from '@m-cafe-app/shared-constants';
...
describe('Database OrderFood model tests', () => {
  ...
  it('OrderFood update test', async () => {
    ...
    orderFood.quantity = 2;
    orderFood.archivePrice = 2;
    orderFood.archiveFoodName = 'тест2';

    await orderFood.save();

    const orderFoodInDB = await OrderFood.findOne({ where: { orderId: order.id, archiveFoodId: food.id } });

    expect(orderFoodInDB?.quantity).to.equal(2);
